    ...
    <view class="u-wrap" :style="{
        ...
        <!-- <view :class="'u-lazy-item-' + elIndex"> -->
            <image :style="{ borderRadius: borderRadius + 'rpx', height: imgHeight }" v-if="!isError"
                class="u-lazy-item" :src="isShow ? image : loadingImg" :mode="mode" @load="imgLoaded"
                @error="loadError" >
            </image>
            <image :style="{ borderRadius: borderRadius + 'rpx', height: '100%', width: '100%' }" class="u-lazy-item1 placeholder error" v-else
