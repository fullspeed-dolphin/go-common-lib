<template>
  <u-popup mode="center" :zoom="zoom" :show="show" :class="[customClass]" :customStyle="{
			borderRadius: '8px', 
			overflow: 'hidden',
			marginTop: `-${addUnit(negativeTop)}`
		}" :closeOnClickOverlay="closeOnClickOverlay" :safeAreaInsetBottom="false" :duration="duration" @click="clickHandler">
      ...
      <template v-else>
        ...
        <view class="u-modal__button-group" :style="{
					...
					<template v-if="showCancelButton">
						<button v-if="openType === 'contact'" open-type="contact" class="u-modal__button-group__wrapper u-modal__button-group__wrapper--cancel" :hover-stay-time="150" hover-class="u-modal__button-group__wrapper--hover"
							:class="[showCancelButton && !showConfirmButton && 'u-modal__button-group__wrapper--only-cancel']" @tap="cancelHandler">
							<text class="u-modal__button-group__wrapper__text" :style="{
									color: cancelColor
								}">{{ cancelText }}</text>
						</button>

						<view v-else class="u-modal__button-group__wrapper u-modal__button-group__wrapper--cancel" :hover-stay-time="150" hover-class="u-modal__button-group__wrapper--hover"
							:class="[showCancelButton && !showConfirmButton && 'u-modal__button-group__wrapper--only-cancel']" @tap="cancelHandler">
							<text class="u-modal__button-group__wrapper__text" :style="{
									color: cancelColor
								}">{{ cancelText }}</text>
          ...
          <u-line direction="column" v-if="showConfirmButton && showCancelButton" style="height:48px;"></u-line>
          <view class="u-modal__button-group__wrapper u-modal__button-group__wrapper--confirm" :hover-stay-time="150" hover-class="u-modal__button-group__wrapper--hover"
            :class="[!showCancelButton && showConfirmButton && 'u-modal__button-group__wrapper--only-confirm']" v-if="showConfirmButton" @tap="confirmHandler">
...
export default {
  ...
  computed: {
    contentStyleCpu() {
      ...
      style.paddingTop = `${this.title ? 20 : 25}px`;
      return style;
    },
  },
  methods: {
