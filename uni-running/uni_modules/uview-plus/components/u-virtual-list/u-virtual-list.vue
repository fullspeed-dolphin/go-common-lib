<template>
	<view class="u-virtual-list" :style="{ height: addUnit(height) }" ref="container">
		<scroll-view class="virtual-scroll-container" :scroll-y="true" :scroll-top="scrollTop" :style="{ height: '100%' }"
			@scroll="handleScroll">
			<view class="scroll-content">
				...
				<!-- 可见项 -->
				<view v-for="item in visibleItems" :key="getItemKey(item)" class="list-item"
					:style="{ height: itemHeight + 'px' }">
					<slot name="item" :item="item" :index="item._virtualIndex"></slot>
				</view>
