<template>
	<view :class="{ 'xz-slider-range--disabled': $props.disabled }" class="xz-slider-range">
		<view class="xz-slider-range__inner" id="sliderRange">
			...
			<view v-for="(item, index) in sliderBlockCtx.blocks" :key="index" :style="item.style"
				class="xz-slider-range__controls" @touchmove="blockEvent($event, index)" @touchstart="showHandle(0)"
				@touchend="showHandle(1)">
				<!-- 滑块值提示 -->
          ...
          <text>总{{ $props.total }}</text>
				...
				<!-- 显示的滑块数字 -->
				<view :class="{'xz-slider-range__block--decoration': sliderBlockCtx.decoration}" class="xz-slider-range__block"
					vif="isShow">{{ item.hint+1 }}/{{$props.max}}</view>
				<!-- 显示的圆心滑块 -->
