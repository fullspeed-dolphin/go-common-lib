...
// idTypeOptions, countryOptions, tshirtSizeOptions
	...
	const forceRegenerateBirthday = ref(false);

	watch(
		() => form.idCardNumber,
		(newIdCard) => {
			//console.log('身份证号码变化:',form, newIdCard);
			// 只在证件类型为"身份证"时才自动解析
			if (form.idType !== "身份证") {
				return;
			}
			...
			try {
				...
				if (birthdayStr && birthdayStr.length === 8) {
					...
					if (date.isValid()) {
						...
						// 如果强制重新生成标志为true，或者当前出生日期是之前自动生成的，或者是空的，则更新
						if (
							forceRegenerateBirthday.value ||
							form.birthday === lastAutoGeneratedBirthday.value ||
							!form.birthday
						) {
							isAutoGeneratingBirthday.value = true;
							// form.birthday = formattedBirthday
							// console.log("自动解析出生日期:", formattedBirthday);
							lastAutoGeneratedBirthday.value = formattedBirthday;
							forceRegenerateBirthday.value = false;
	...
	watch(
		...
		(newIdType, oldIdType) => {
			...
			// 如果改为身份证，且身份证号码已填写，尝试解析出生日期
				...
				// 强制重新提取出生日期，即使之前是手动选择的
				forceRegenerateBirthday.value = true;
				// 触发身份证号码的 watch
	...
	const rules = {
		...
		{
			validator: validateIdCardIfNeeded,
			trigger: ["blur"]
    }],
		birthday1: [{
			required: true,
