<template>
  <view class="page">
    ...
    <view class="tab-container">
      <u-tabs lineHeight="2" :duration="0" :inactiveStyle="{ color: '#000' }" :activeStyle="{ color: '#FF8C00' }" :list="tab.items" @change="changeTab" :scrollable="false" keyName="label" lineColor="#FF8C00" />
    </view>
    <mescroll-uni @init="mescrollInit" @down="downCallback" @up="getList" top="300">
      <view class="order-list">
        <view class="order-item" v-for="order in orders" :key="order.id">
          ...
          <view class="" @click="viewDetail(order)">
            <EventItem from="order" :item="order.event_info" direction="row" :showButton="false" />
...
const getList = (mescroll) => {
  ...
  proxy.$axios
    ...
    .then((res) => {
      mescroll.endSuccess(res.orders.length);
