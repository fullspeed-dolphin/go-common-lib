<template>
  <view style="padding: 24rpx;padding-bottom: 160rpx;background:#f3f3f3;min-height: 100vh;box-sizing: border-box;">
      ...
      <up-form :model="form" ref="uForm" :rules="rules" labelWidth="auto">
        ...
        <up-form-item label="活动地址" prop="event_location" required>
          <view class="pickermap" style="padding:0;">
            <u-cell :border="false" :value="form.event_location || '请选择地址'" @click="handleChooseLocation()" isLink />
          </view>
        </up-form-item>

        <TagForm title="活动项目" v-model="form.event_projects" required prop="event_projects" name="event_projects" @input="validateField('event_projects')" maxlength="50" placeholder="请添加活动项目" />

        <u-form-item label="活动时间" prop="event_time" required>
          <PickerTime v-model="form.event_time" mode="datetime" :minDate="new Date().getTime()" :maxDate="maxDate" :filter1="timeFilter" :border="false" returnType="YYYY-MM-DD hh:mm" placeholder="请选择时间" :title="null"
            @change="validateField('event_time')" />
        </u-form-item>
        ...
        <TagForm title="参赛包领取地址" v-model="form.racekit_pickup_address" required prop="racekit_pickup_address" name="racekit_pickup_address" @input="validateField('racekit_pickup_address')" maxlength="500"
          placeholder="请添加参赛包领取地址" />
    ...
    <view class="section-bottom">
      <u-button type="primary" color="#FF8C00" shape="circle" :disabled="isSubmitting" customStyle="height: 84rpx; width: 100%;" @click="submitForm()">
        {{computedSubmitBtn}}
      </u-button>
    </view>
  </view>
...
const getDetail = (page) => {
  ...
  request.get(`/event-api/fsc_events/${routerParams.value.id}`).then((res) => {
    console.log("res=====>", res);
    ...
    Object.assign(form.value, {
      ...
      event_time:
        res.event_time != null
          ? String(dayjs(res.event_time).valueOf())
          : form.value.event_time,
      // ensure these fields are strings so validator treats them as filled
      capacity:
        res.capacity != null ? String(res.capacity) : form.value.capacity,
      multi_package:
        res.multi_package != null
          ? String(res.multi_package)
          : form.value.multi_package,
