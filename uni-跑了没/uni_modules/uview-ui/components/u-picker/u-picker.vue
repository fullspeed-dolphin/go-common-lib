<template>
  <u-popup :show="show" @close="closeHandler">
    <view class="u-picker">
      ...
      <picker-view class="u-picker__view" :indicatorStyle="`height: ${$u.addUnit(itemHeight)}`" :value="innerIndex" :immediateChange="immediateChange" :style="{
        ...
        <picker-view-column v-for="(item, index) in innerColumns" :key="index" class="u-picker__view__column">
          <text v-if="$u.test.array(item)" class="u-picker__view__column__item u-line-1" v-for="(item1, index1) in item" :key="index1" :style="{
