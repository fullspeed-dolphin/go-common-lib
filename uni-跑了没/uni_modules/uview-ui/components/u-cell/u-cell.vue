<template>
  <view class="u-cell" :class="[customClass, {'u-cell--required': required}]" :style="[$u.addStyle(customStyle)]" :hover-class="(!disabled && (clickable || isLink)) ? 'u-cell--clickable' : ''" :hover-stay-time="250"
    @tap="clickHandler">
    <view class="u-cell__body" :class="[ center && 'u-cell--center', size === 'large' && 'u-cell__body--large']">
      <view class="u-cell__body__content">
        ...
        <view class="u-cell__title">
          <slot name="title">
            <text v-if="title" class="u-cell__title-text" :style="[titleTextStyle]" :class="[disabled && 'u-cell--disabled', size === 'large' && 'u-cell__title-text--large']">{{ title }}</text>
          </slot>
          <slot name="label">
      ...
      <!-- <slot name="value">
        <text class="u-cell__value" :class="[disabled && 'u-cell--disabled', size === 'large' && 'u-cell__value--large']" v-if="!$u.test.empty(value)">{{ value }}</text>
      </slot> -->

      <view class="u-cell__value" :class="[disabled && 'u-cell--disabled', size === 'large' && 'u-cell__value--large']">
        <block v-if="!$u.test.empty(value)">{{ value }}</block>
        <slot v-else></slot>
      </view>
      ...
      <view class="u-cell__right-icon-wrap" v-if="$slots['right-icon'] || isLink" :class="[`u-cell__right-icon-wrap--${arrowDirection}`]">
        ...
        <u-icon v-else :name="rightIcon" :custom-style="rightIconStyle" :color="disabled ? '#c8c9cc' : 'info'" :size="size === 'large' ? 18 : 14"></u-icon>
      </view>
    </view>
