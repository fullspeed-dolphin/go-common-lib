    	...
    	<view class="flex-row"  @click="$u.route(`pagesSub/offlineEvents?id=${detail.event_id}`)">
    		...
    		<view class="text">
    			<view class="flex-between-center" style="width: 420rpx;">
    				<view class="">
    					<view class="name ellipsis2">{{detail.event_info.name || '活动'}}</view>
    					<view class="city ellipsis">创建时间: {{detail.created_at}}</view>
    				</view>
    			</view>
    			<view class="flex-between-center">
    				<view class="flex-row">
							报名费用：
    					<view class="ellipsis" style="color:red;">￥{{detail.amount_yuan}}</view>
    				</view>
    				<u-button v-if="detail.status === 'PND'" type="primary" color="#19be6b" shape="circle" size="mini" @click="payOrder(item)">
    					微信支付
    				</u-button>
    			</view>
    		</view>
