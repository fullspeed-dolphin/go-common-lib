<template>
	<view class="index-page">
		<view class="section-search">
			<u-search placeholder="搜索" shape="square" bgColor="#eff7fe" searchIconColor="#2f4f65" borderColor="#2f4f65"
				:showAction="false" v-model="searchTxt" @change="$u.debounce(confirmSearch, 300)"></u-search>
		</view>
		<mescroll-uni ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="getList" top="120rpx">
			<view class="" style="position:relative;z-index: 10;">
				...
				<SectionPanel title="摄影作品" type="photo">
					<PhotoItem v-for="(item, index) in dataList" :data="item" :key="index" />
				</SectionPanel>
			</view>
		</mescroll-uni>

		<tabbar type="photoList" />
	</view>
</template>
<script>
...
<style lang="less" scoped>
	.index-page {
		...
		&::before {
			...
			width: 100%;
			height: 390rpx;
			background: linear-gradient(#dcfbfc, #f8fefe);
		}
	}
	...
	::v-deep {
		...
		.u-search {
			...
			&::before {
				...
				width: 100%;
				height: 120rpx;
				background: linear-gradient(#e0fbfc, #e6fcfd);
			}
