<template>
  <view>
    <u-cell :title="title" :required="required">
      <uni-file-picker v-model="fileList" :file-mediatype="accept" :limit="limit" :mode="mode" @select="selectFile" @delete="deleteFile" :auto-upload="false" :readonly="readonly" :imageStyles="imageStyles"
        :disable-preview="disablePreview" :delIcon="delIcon" @fail="fail" />
    </u-cell>
  </view>
</template>
<script>
...
export default {
        ...
        if (val) {
          ...
          this.fileList = list.map((item) => {
            if (typeof item === "string") {
              item = { name: item, url: item, extname: "" };
            }
  ...
  methods: {
    ...
    async selectFile(event) {
      ...
      const promiseList = event.tempFiles.map((item) =>
    ...
    async uploadFile(file) {
      ...
      if (platform === "client") {
        header = {
          ...
          "Supply-Authorization": uni.getStorageSync("token"),
      ...
      return new Promise((resolve, reject) => {
        uni.uploadFile({
          url:
            baseLink +
            `/file/${platform === "admin" ? "admin" : "mina"}/upload`,
          filePath: filePath,
          name: "file",
          header: {
